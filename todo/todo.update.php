...
class TodoUpdate extends database{
private $dbconnect;
...
function selectTodo(){
    ...
    $stmt= $connection->prepare('SELECT * FROM todo WHERE id=:id');
    ...
    if ($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        if(empty($error)){
            $stmt= $connection->prepare("UPDATE todo SET name=:name,date=:date WHERE id=:id");
...
<html lang="en">
  ...
  <p>
    <a href="TodoView.php" class='btn btn-secondary'>Go back to ptoducts</a>
  </p>
    <h1>Update product <b> <?php $test= new TodoUpdate();
                              $value=  $test->selectTodo();
...
<?php endif ; ?>
 ...
 <!-- Display the  values -->
  ...
  <div class="mb-3">
    ...
    <input type="datetime-local"  name="date" class="form-control" value="<?php $test= new TodoUpdate();
    $value=  $test->selectTodo();
     echo $value['date'] ?> ">
  </div>
